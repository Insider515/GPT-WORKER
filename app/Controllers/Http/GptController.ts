...
interface MyBatch {
    [key: string]: string; // Здесь указывается ожидаемая структура пакета
  }
...
export default class GptController {
    ...
    ].filter(key => typeof key === 'string') as string[];; // Масив ключів API
...
*/
    ...
    static async main(translatedObject: MyBatch) {
        let currentTokens = 0;
        let currentBatch: MyBatch = {};
        let batches: MyBatch[] = []; // Указываем тип массива как MyBatch[]
